...
import com.doradosystems.mis.worker.config.WorkerPoolConfiguration;
...
public class EnvironmentModule {
  ...
  @Singleton
  @Nonnull
  public BlockingQueue<Runnable> provideJobQueue(WorkerPoolConfiguration config) {
    return new LinkedBlockingQueue<>(config.getMaxJobsInQueue());
  }
  
  @Provides
  @Singleton
  public ExecutorService provideExecutorService(WorkerPoolConfiguration config, @Named(JOB_QUEUE) BlockingQueue<Runnable> jobQueue) {
    ...
    final ExecutorService executorService = environment.executorService("MIS Worker Service", factory)
        ...
        .minThreads(config.getMinWorkers())
        .maxThreads(config.getMaxWorkers())
