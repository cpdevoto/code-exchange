...
public class DdeCrawlerJobImpl implements DdeCrawlerJob {
  private static final Logger log = LoggerFactory.getLogger(DdeCrawlerJobImpl.class);
  ...
  @Inject
  public DdeCrawlerJobImpl(@Nonnull MetricRegistry metrics, @Nonnull OperatorDao operatorDao,
      @Nonnull NpiDao npiDao) {
    this.operatorDao = requireNonNull(operatorDao);
    this.npiDao = requireNonNull(npiDao);
    
    this.jobExecutionTimes = metrics.timer(name(DdeCrawlerJobImpl.class, "jobExecutionsTimes"));
    
    metrics.register(name(DdeCrawlerJobImpl.class, "lastExecutionTime"), new Gauge<String>() {
      @Override
    ...
    metrics.register(name(DdeCrawlerJobImpl.class, "tasksGenerated"), new Gauge<Integer>() {
      @Override
